...
int main(void)
{
  ...
  double Q_xx[q],Q_yy[q],Q_xy[q],cc_xx[q],cc_xy[q],cc_yy[q],pi_xx,pi_yy,pi_xy;
	double tmp1, tmp2, tmp3, invC, rhoAvg, feq[q], f_neq_[q],f_neq[q], fx_t, fy_t, Cd, Cl;
 ...
 for (j=1; j<=ny; j++)
 {
   ux_a[j]=6*uInlet*(ny-(j-0.5))*(j-0.5)/(ny*ny);
    ...
    cc_yy[a] = ey[a]*ey[a];
    cc_xy[a] = ex[a]*ey[a];
    //printf("%d\t%5.2f\t%5.2f\t%5.2f\t%5.2f\t%5.2f\t%5.2f\n",i,Q_xx[a],Q_yy[a],Q_xy[a],cc_xx[a],cc_yy[a],cc_xy[a]);
	}
	...
	for (ts=0; ts<=time; ts++)
	{
      ...
      for(a=0;a<9;a++)
			{
				...
				feq[a] = wt[a]*rho[i][j]*(1 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2);
      }
      ...
      f_neq_[5] = f_neq_[7];// + 0.5*(f_neq_[4]-f_neq_[2]);
      f_neq_[1] = f_neq_[3];
      f_neq_[8] = f_neq_[6];// + 0.5*(f_neq_[2]-f_neq_[4]);
      ...
      for(a=0;a<9;a++)
			{        
				pi_xx += cc_xx[a]*f_neq_[a];
        pi_yy += cc_yy[a]*f_neq_[a];
        pi_xy += cc_xy[a]*f_neq_[a];
			}
      
			for(a=0;a<9;a++)
			{        
				f_neq[a] = 4.5*wt[a]*(Q_xx[a]*pi_xx + 2.0*Q_xy[a]*pi_xy + Q_yy[a]*pi_yy);
				f[a][i][j] = feq[a] + f_neq[a];
			}
      //=====================
			//Outlet
			i = nx;
      ...
      f_neq_[3] = f_neq_[1];
      f_neq_[6] = f_neq_[8];// + 0.5*(f_neq_[4]-f_neq_[2]);
      f_neq_[7] = f_neq_[5];// + 0.5*(f_neq_[2]-f_neq_[4]);
